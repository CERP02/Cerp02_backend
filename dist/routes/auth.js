...
router.post("/register", async (req, res) => {
    ...
    const validRoles = ["user", "responder", "admin", "superadmin"];
    ...
    try {
        ...
        if (!passwordMatch) {
            ...
            if (newAttempts >= 5) {
                ...
                await db_1.default.query("UPDATE users SET login_attempts = $1, locked_until = $2 WHERE id = $3", [newAttempts, lockTime, user.id]);
                res.status(403).json({ error: "Too many failed attempts. Account locked for 30 minutes." });
            }
            else {
                await db_1.default.query("UPDATE users SET login_attempts = $1 WHERE id = $2", [newAttempts, user.id]);
                res.status(401).json({ error: "Invalid email or password" });
            }
            return;
        }
        // On success, reset login attempts
        await db_1.default.query("UPDATE users SET login_attempts = 0, locked_until = NULL WHERE id = $1", [user.id]);
        // Ensure the JWT secret is configured
        if (!JWT_SECRET) {
...
router.post("/forgot-password", async (req, res) => {
    ...
    try {
        const result = await db_1.default.query("SELECT id FROM users WHERE email = $1", [email.toLowerCase()]);
        if (result.rows.length === 0) {
        ...
        await db_1.default.query("UPDATE users SET reset_token = $1, reset_token_expiry = $2 WHERE email = $3", [token, expiry, email.toLowerCase()]);
...
router.post("/reset-password", async (req, res) => {
    ...
    try {
        const result = await db_1.default.query("SELECT id FROM users WHERE reset_token = $1 AND reset_token_expiry > NOW()", [token]);
        if (result.rows.length === 0) {
        ...
        await db_1.default.query("UPDATE users SET password_hash = $1, reset_token = NULL, reset_token_expiry = NULL, login_attempts = 0, locked_until = NULL WHERE id = $2", [hash, userId]);
